...
class ModelManager:
    def __init__(self, model_meta, use_gcp_index: bool = False, load_all: bool = False):
        ...
        self.models_clustering = sorted(
            set(model_meta["model_meta"].keys()) - set(["BM25"])
        )
        ...
        if load_all:
            ...
            # Load BM25 indices - DISABLED: each index is ~3.5G, total ~10.5G
    ...
    def load_local_index(self, model_name, corpus, embedbs=32) -> DistributedIndex:
        ...
        if (model_name in self.loaded_indices) and (
            corpus in self.loaded_indices[model_name]
        ):
    ...
    def load_bm25_index(self, model_name: str, corpus: str, limit=None) -> BM25Index:
        if model_name in self.loaded_indices:
            if corpus in self.loaded_indices[model_name]:
                return self.loaded_indices[model_name][corpus]
        index = BM25Index(model_name=model_name, corpus=corpus, limit=limit)
        index.load_index()
        self.loaded_indices.setdefault(model_name, {})
        self.loaded_indices[model_name][corpus] = index
        return index
    ...
    def retrieve_draw(self, corpora=["wikipedia", "arxiv", "stackexchange"]):
        if "retrieval" not in self.loaded_samples:
            ...
            self.loaded_samples["retrieval"]["wikipedia"] = load_dataset(
                "mteb/nq", "queries", split="queries"
            )["text"]
            self.loaded_samples["retrieval"]["arxiv"] = load_dataset(
                "mteb/arena-arxiv-7-2-24-samples", split="train"
            )["query"]
            # Use MTEB's standard Lotte dataset (no script required)
            lotte_queries = []
            for config in [
                ...
                ds = load_dataset("mteb/lotte", config, split="test")
                lotte_queries.extend(ds["text"])
            self.loaded_samples["retrieval"]["stackexchange"] = lotte_queries
        ...
        if "clustering" not in self.loaded_samples:
            ...
            for i in range(10000):
                ...
                sampled_categories = random.sample(
                    list(CLUSTERING_CATEGORIES.keys()), n_categories
                )
                sampled_items = []
                for category in sampled_categories:
                    ...
                    sampled_items.extend(
                        random.sample(
                            CLUSTERING_CATEGORIES[category],
                            random.randint(
                                2, min(len(CLUSTERING_CATEGORIES[category]), 8)
                            ),
                        )
                    )
    ...
    def sts_draw(self):
        if "sts" not in self.loaded_samples:
            ...
            self.loaded_samples["sts"] = load_dataset(
                "sentence-transformers/all-nli", "triplet", split="test"
            )
    ...
    def retrieve_parallel(self, prompt, corpus, model_A, model_B):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(self.retrieve, prompt, corpus, model)
    ...
    def retrieve(self, query, corpus, model_name, topk=1):
                ...
                return [
                    [
                        ...
                        corpus_format.format(
                            title=docs[0][0]["title"], text=docs[0][0]["text"]
                        ),
                    ]
                ]
        ...
        if f"instruction_query_{corpus}" in self.model_meta[model_name]:
            kwargs["instruction"] = self.model_meta[model_name][
                f"instruction_query_{corpus}"
            ]
        ...
        if self.use_gcp_index:
            ...
            docs = [
                [
                    query,
                    corpus_format.format(
                        title=docs[0].get("title", ""), text=docs[0]["text"]
                    ),
                ]
            ]
        else:
            ...
            docs = [
                [
                    query,
                    corpus_format.format(
                        title=docs[0][0].get("title", ""), text=docs[0][0]["text"]
                    ),
                ]
            ]
        return docs
    ...
    def clustering(
        ...
        if len(queries) == 1:
            ...
            if dim_method == "UMAP":
                ...
                vis_dims = TSNE(
                    n_components=2, perplexity=min(30.0, len(queries) // 2)
                ).fit_transform(emb)
            else:
            ...
            if ncluster > 1:
                if clustering_method == "MiniBatchKMeans":
                    data["cluster"] = (
                        MiniBatchKMeans(
                            n_clusters=ncluster, n_init="auto", random_state=0
                        )
                        .fit_predict(emb)
                        .tolist()
                    )
                    ...
                    data["cluster"] = (
                        KMeans(n_clusters=ncluster, n_init="auto", random_state=0)
                        .fit_predict(emb)
                        .tolist()
                    )
            ...
            if ncluster > 1:
                ...
                fig = px.scatter(
                    df, x="x", y="y", template="plotly_dark", hover_name="txt"
                )
            ...
            if dim_method == "UMAP":
                ...
                vis_dims = TSNE(
                    n_components=3, perplexity=min(30.0, len(queries) // 2)
                ).fit_transform(emb)
            else:
            ...
            if ncluster > 1:
                ...
                fig = px.scatter_3d(
                    df, x="x", y="y", z="z", template="plotly_dark", hover_name="txt"
                )
        fig.update_traces(
    ...
    def sts_parallel(self, txt0, txt1, txt2, model_A, model_B):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(self.sts, txt0, txt1, txt2, model)
                for model in model_names
            ]
            results = [future.result() for future in futures]
        return results[0], results[1], model_names[0], model_names[1]
    ...
    def sts(self, txt0, txt1, txt2, model_name):
        ...
        cos_sims = np.array([cos_sim_01, cos_sim_02, cos_sim_12])
        cos_sims = (
            1 - (cos_sims - cos_sims.min()) / (cos_sims.max() - cos_sims.min()) * 0.5
        )
        ...
        distances = [f"{round(c)}", f"{round(b)}", f"{round(a)}"]
        ...
        for i, (x, y) in enumerate(midpoints):
            fig.add_trace(
                go.Scatter(
                    ...
                    textposition="top center",
                    textfont=dict(
                        size=20, color="black", family="Arial", weight="bold"
                    ),
                    showlegend=False,
                    hoverinfo="none",
                )
