...
from ui import (
    build_side_by_side_ui_anon,
    build_side_by_side_ui_anon_sts,
    build_side_by_side_ui_anon_clustering,
    build_side_by_side_ui_named,
    build_side_by_side_ui_named_sts,
    build_side_by_side_ui_named_clustering,
    build_single_model_ui,
    build_single_model_ui_sts,
    build_single_model_ui_clustering,
)
...
def get_credentials():
    ...
    creds_json_str = os.getenv(
        "GCP_CREDENTIALS"
    )  # get json credentials stored as a string
...
# Temporarily disable leaderboard due to plotly compatibility issues
ELO_RESULTS_DIR = None  # os.getenv("ELO_RESULTS_DIR", "./results/latest")
if DEBUG:
...
for model_name in (
    ...
    if model_name in model_meta["model_meta"]:
        model_meta["model_meta"].pop(model_name)
            ...
            if (elo_results_file) and ("retrieval" in elo_results_file):
                with gr.Tab("🏆 Leaderboard", id=3):
                    build_leaderboard_tab(
                        elo_results_file["retrieval"],
                        leaderboard_table_file["retrieval"],
                        task_type="Retrieval",
                    )
        ...
        with gr.Tabs() as tabs_ie:
            ...
            if (elo_results_file) and ("clustering" in elo_results_file):
                with gr.Tab("🏆 Leaderboard", id=8):
                    build_leaderboard_tab(
                        elo_results_file["clustering"],
                        leaderboard_table_file["clustering"],
    ...
    with gr.Tab("☘️ STS", id=10):
        with gr.Tabs() as tabs_vg:
            ...
            if (elo_results_file) and ("sts" in elo_results_file):
                with gr.Tab("🏆 Leaderboard", id=3):
                    build_leaderboard_tab(
                        elo_results_file["sts"],
                        leaderboard_table_file["sts"],
                        task_type="STS",
                    )
